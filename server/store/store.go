...
func (usersMapper) GetAuthUniqueRecord(scheme, unique string) (types.Uid, auth.Level, []byte, time.Time, error) {
	// Preferred canonical form in newer deployments.
	uid, authLvl, secret, expires, err := adp.AuthGetUniqueRecord(scheme + ":" + unique)
	if err != nil {
		return uid, authLvl, secret, expires, err
	}
	if !uid.IsZero() {
		return uid, authLvl, secret, expires, nil
	}

	// Backward compatibility for legacy records where auth.uname was stored without "scheme:" prefix.
	return adp.AuthGetUniqueRecord(unique)
}
